    ...
    return prisma.$transaction(async (prisma) => {
      ...
      await prisma.progress_perjalanan.create({
        data: {
          ...
          time_selesai: null,
          durasi_progress: 0, // Set durasi_progress awal ke 0
        }
      });    
  ...
  const exitProgress = async (progressid) => {
    return prisma.$transaction(async (prisma) => {
      ...
      const progressPerjalananData = participants.map((participant) => ({
        ...
        durasi_progress: formattedDuration, // Simpan durasi dalam format MM:SS
      }));
      ...
      return {
        ...
        time_selesai: timeNow,
        durasi_progress: formattedDuration, // Kembalikan durasi dalam format MM:SS
      };
    });
  };
